            ...
            <div className={`md:py-4 md:px-6 sm:p-3 py-3 border-b-4 border-card-color bg-body-color ${headerFix ? 'sticky top-0 z-[2] xl:shadow-none shadow-lg' : ''}`}>
                ...
                <div className='flex items-center gap-3 sm:pe-4 pe-2'>
                    ...
                    <div ref={searchRef} className='relative px-4 flex-1 md:block hidden'>
                        <input ref={inputRef} type="text" placeholder="Enter your search key word" onClick={toggleSearchBar} className={`w-full py-[6px] px-[12px] bg-card-color rounded-md border border-border-color focus:outline-0 ${searchBar ? 'z-[5] relative' : ''}`} />
                        <div className={`bg-card-color border border-dashed border-border-color text-font-color xl:absolute fixed xl:left-0 left-30 xl:top-[40px] top-[68px] z-[5] xl:w-full w-[calc(100%-60px)] rounded-xl p-6 transition-all duration-300 origin-top ${searchBar ? 'opacity-1 visible scale-y-100' : 'opacity-0 invisible scale-y-0'}`}>
                    ...
                    <div className='flex items-center ms-auto'>
                        ...
                        <div className='relative group flex'>
                            ...
                            <div className='bg-card-color text-font-color rounded-xl overflow-hidden md:w-[240px] w-[calc(100%-30px)] shadow-shadow-lg md:absolute fixed md:right-0 right-15 md:top-full top-[55px] origin-top-right z-[1] opacity-0 invisible scale-0 transition-all duration-300 group-hover:opacity-100 group-hover:visible group-hover:scale-100'>
                                ...
                                <Link href="/auth/sign-in" className='bg-secondary uppercase text-[14px]/[20px] text-white py-5 px-10 text-center w-full inline-block'>
                                    Sign Out
                                </Link>
                            </div>
                        </div>
