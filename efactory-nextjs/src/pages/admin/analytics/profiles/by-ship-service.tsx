	...
	const processedData = useMemo(() => {
		...
		if (sortedData.length > 10) {
			...
			const othersData = others.reduce((acc, item) => ({
				...
				lines: (acc.lines || 0) + (item.lines || 0),
				packages: (acc.packages || 0) + (item.packages || 0),
				units: (acc.units || 0) + (item.units || 0),
			year_1: {
				orders: (acc.year_1?.orders || 0) + (item.year_1?.orders || 0),
				lines: (acc.year_1?.lines || 0) + (item.year_1?.lines || 0),
				packages: (acc.year_1?.packages || 0) + (item.year_1?.packages || 0),
				units: (acc.year_1?.units || 0) + (item.year_1?.units || 0)
			},
			year_2: {
				orders: (acc.year_2?.orders || 0) + (item.year_2?.orders || 0),
				lines: (acc.year_2?.lines || 0) + (item.year_2?.lines || 0),
				packages: (acc.year_2?.packages || 0) + (item.year_2?.packages || 0),
				units: (acc.year_2?.units || 0) + (item.year_2?.units || 0)
			}
			}), {
				id: 'Others',
				name: 'Others',
