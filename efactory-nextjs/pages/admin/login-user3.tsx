...
function LoginUserPageInner() {
	...
	const selectedAccount = accounts.find(acc => acc.username === selectedUsername);
			...
			<div className='container-fluid pb-8'>
				<div className='max-w-[1600px] mx-auto'>
					<div className='flex gap-6'>
						{/* Master List */}
						<div className={`transition-all duration-300 ${showSidebar ? 'w-1/2' : 'w-full'}`}>
...
function AccountTableRow({ account, index, isSelected, onSelect, onProceed, submitting, onShowDetails }: {
...
function AccountDetailPanel({ account, onClose, onProceed, submitting, allAccounts }: {
	account: AvailableAccountItem;
	onClose: () => void;
	onProceed: () => void;
	submitting: boolean;
	allAccounts: AvailableAccountItem[];
}) {
	const initials = getInitials(account.username);
	const locTokens = String(account.location || '')
		.split(',')
	...
	// Find related accounts (same company)
	const relatedAccounts = allAccounts.filter(acc => 
		acc.company === account.company && acc.username !== account.username
	);

	// Calculate account metrics
	const totalLocations = locTokens.length;
	const isEDI = account.is_EDI || false;

	return (
		<div className='card bg-card-color rounded-lg border border-dashed border-border-color h-full flex flex-col'>
			{/* Header */}
			<div className='flex items-center justify-between p-6 border-b border-border-color'>
				<div className='flex items-center gap-3'>
					<div className='w-[48px] h-[48px] bg-gradient-to-br from-primary to-secondary rounded-xl flex items-center justify-center text-white text-[18px] font-bold'>
						{initials}
					</div>
					<div>
						<h3 className='text-[18px]/[24px] font-bold text-font-color'>{accountNumber}</h3>
						<p className='text-[14px]/[20px] text-font-color-100'>{companyName}</p>
					</div>
				</div>
				<button
					onClick={onClose}
					className='w-[32px] h-[32px] rounded-lg bg-primary-10 text-primary hover:bg-primary hover:text-white transition-colors flex items-center justify-center'
				>
			...
			<div className='flex-1 p-6 space-y-6 overflow-y-auto'>
				{/* Account Type */}
				<div>
					<h4 className='text-[14px]/[18px] font-semibold text-font-color mb-3 flex items-center gap-2'>
						<IconShield className='w-[16px] h-[16px]' />
						Account Type
					</h4>
					<div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg ${
						isEDI ? 'bg-success-10 text-success' : 'bg-primary-10 text-primary'
					}`}>
						{isEDI ? (
							<>
								<IconShield className='w-[16px] h-[16px]' />
								<span className='font-medium'>EDI Account</span>
							</>
						) : (
							<>
				...
				<div>
					<h4 className='text-[14px]/[18px] font-semibold text-font-color mb-3 flex items-center gap-2'>
						<IconMapPin className='w-[16px] h-[16px]' />
						Locations ({totalLocations})
					</h4>
					{locTokens.length > 0 ? (
						<div className='space-y-2'>
							{locTokens.map((location, index) => (
								<div key={location} className='flex items-center gap-3 p-2 rounded-lg bg-primary-5'>
						...
						<div className='text-center py-4 text-font-color-100'>
							<IconMapPin className='w-[32px] h-[32px] mx-auto mb-2 opacity-50' />
				...
				{relatedAccounts.length > 0 && (
					<div>
						<h4 className='text-[14px]/[18px] font-semibold text-font-color mb-3 flex items-center gap-2'>
							<IconBuilding className='w-[16px] h-[16px]' />
							Related Accounts ({relatedAccounts.length})
						</h4>
						<div className='space-y-2'>
							{relatedAccounts.slice(0, 3).map((relatedAccount) => (
								<div key={relatedAccount.username} className='flex items-center gap-3 p-2 rounded-lg bg-primary-5'>
									<div className='w-[24px] h-[24px] bg-gradient-to-br from-primary to-secondary rounded-md flex items-center justify-center text-white text-[10px] font-bold'>
										{getInitials(relatedAccount.username)}
									</div>
									<div className='flex-1 min-w-0'>
										<div className='text-[12px]/[16px] font-medium text-font-color truncate'>
											{relatedAccount.username}
										</div>
										<div className='text-[10px]/[14px] text-font-color-100'>
											{relatedAccount.is_EDI ? 'EDI' : 'Standard'}
										</div>
									</div>
								</div>
							))}
							{relatedAccounts.length > 3 && (
								<div className='text-center py-2 text-[12px]/[16px] text-font-color-100'>
									+{relatedAccounts.length - 3} more accounts
								</div>
							)}
						</div>
					</div>
				...
				<div>
					<h4 className='text-[14px]/[18px] font-semibold text-font-color mb-3 flex items-center gap-2'>
						<IconActivity className='w-[16px] h-[16px]' />
						Account Statistics
					</h4>
					<div className='grid grid-cols-2 gap-4'>
						<div className='text-center p-3 rounded-lg bg-primary-5'>
							<div className='text-[20px]/[24px] font-bold text-primary'>{totalLocations}</div>
							<div className='text-[11px]/[14px] text-font-color-100'>Locations</div>
						</div>
						<div className='text-center p-3 rounded-lg bg-success-5'>
							<div className='text-[20px]/[24px] font-bold text-success'>{isEDI ? 'Yes' : 'No'}</div>
							<div className='text-[11px]/[14px] text-font-color-100'>EDI Enabled</div>
						</div>
					</div>
				</div>
			</div>
