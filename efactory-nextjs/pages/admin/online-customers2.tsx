...
function OnlineCustomersPageInner() {
	...
	const [analyticsView, setAnalyticsView] = useState<'browsers' | 'companies' | 'activity'>('browsers');
	...
	// Activity simulation (based on user characteristics)
	const recentActivity = rows
		.map(customer => ({
			username: customer.username,
			company: customer.company_name,
			activityScore: (customer.is_master ? 90 : 70) + (customer.location ? customer.location.split(',').length * 10 : 0),
			lastSeen: new Date(Date.now() - Math.random() * 30 * 60 * 1000), // Random within last 30 minutes
			isMaster: customer.is_master,
			browser: customer.short_browser
		}))
					...
					<div className='flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-6'>
						<div>
							...
							<p className='text-font-color-100 text-[16px]/[24px]'>
								Real-time analytics and comprehensive customer session tracking
							</p>
						</div>
					...
					<div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6'>
						...
						<div className='card bg-card-color rounded-xl p-6 border border-dashed border-border-color'>
							<div className='flex items-center justify-between mb-4'>
								<div className='flex items-center gap-3'>
									<div className='w-[32px] h-[32px] bg-primary-10 rounded-lg flex items-center justify-center'>
										<IconActivity className='w-[16px] h-[16px] text-primary' />
									</div>
									<h3 className='text-[16px]/[22px] font-semibold text-font-color'>
										{analyticsView === 'browsers' && 'Browser Distribution'}
										{analyticsView === 'companies' && 'Company Activity'}
										{analyticsView === 'activity' && 'Recent Activity'}
									</h3>
								</div>
								<div className='flex items-center gap-1'>
									<button
										onClick={() => setAnalyticsView('browsers')}
										className={`px-3 py-1 rounded-md text-[12px]/[16px] font-medium transition-colors ${
											analyticsView === 'browsers' 
							...
							<div className='space-y-3'>
								{analyticsView === 'browsers' && topBrowsers.map(([browser, count], index) => (
										...
										<div className='flex items-center gap-2'>
											<div className='w-[60px] bg-success-10 rounded-full h-2'>
												<div 
													...
													style={{ width: `${(count / topBrowsers[0][1]) * 100}%` }}
												/>
											</div>
								...
								{analyticsView === 'companies' && topCompanies.map(([company, count], index) => (
								...
								{analyticsView === 'activity' && recentActivity.map((item, index) => (
										...
										<div className='flex items-center gap-2'>
											<div className='w-[60px] bg-warning-10 rounded-full h-2'>
												<div 
													...
													style={{ width: `${(item.activityScore / recentActivity[0].activityScore) * 100}%` }}
												/>
											</div>
											<div className='text-right'>
												<div className='text-[12px]/[16px] text-font-color-100 font-mono'>{item.activityScore}</div>
												<div className='text-[10px]/[12px] text-font-color-100'>score</div>
											</div>
										</div>
									</div>
								))}
