...
const AddShipConfirmScheduler = props => {
  ...
  const handleModalOpening = useCallback(
    () => {
      let { formDataInitial, isAdd, isPoReceipt } = propsRef.current
      if (!isAdd) {
        ...
        setProduct_group(isPoReceipt? 'ALL': '',)
        setEnvironment('',)
  ...
  function onAddSchedulerClicked () {
    let { settingsActions, isPoReceipt } = props
    if (isPoReceipt ){
      settingsActions.add3PLPOReceiptScheduler({
        product_group,
        environment,
      ...
      settingsActions.add3PLShipConfirmScheduler({
        product_group,
        environment,
        time : currentTimeIsoFormat,
        one_time
  ...
  function onUpdateSchedulerClicked () {
    let { settingsActions, isPoReceipt } = props
    if (isPoReceipt ){
      settingsActions.update3PLPOReceiptScheduler({
        product_group,
        environment,
        time : currentTimeIsoFormat,
        id
      ...
      settingsActions.update3PLShipConfirmScheduler({
        product_group,
        environment,
        time : currentTimeIsoFormat,
        id
              ...
              >
                ...
                <div className="col-md-6">
                  ...
                  >
                    ...
                    <option value="NORTONCORE">
                      NORTONCORE
                    </option>
                    <option value="YELLOWBOX">
                      YELLOWBOX
                    </option>
                  </select>
                </div>
...
AddShipConfirmScheduler.propTypes = {
...
export default AddShipConfirmScheduler