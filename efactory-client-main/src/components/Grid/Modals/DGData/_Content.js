...
const DGDataModal = (props) => {
  ...
  useEffect(
    () => {
      global.$('#dg-data').on('show.bs.modal', handleModalOpening )
      global.$(".draggable-modal").draggable({ handle: ".modal-header"})
      return () => {
        global.$('#dg-data').off('show.bs.modal', handleModalOpening )
      }
    },
    []
  )
  ...
  function saveChangesThisWh () {
    setLoading2(true)
    props.gridActions.postDGData( false ).then(
      ({ success }) => {
        setLoading2(false)
        if( success ){
          global.$('#dg-data').modal('hide')
        }
    })
  }
  ...
  function saveChangesAllWh () {
    setLoading1(true)
    props.gridActions.postDGData( true ).then(
      ({ success }) => {
        setLoading1(false)
        if( success ){
          global.$('#dg-data').modal('hide')
