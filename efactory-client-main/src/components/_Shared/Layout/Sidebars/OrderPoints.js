...
const OrderpointsSidebar = props => {
  function processSidebarConfig () {
    let routeAppIdsTable = createRouteAppIdsConfig()
    let { searchBox, menus } = sidebarConfig[ 'orderpoints' ]
    menus = [ ...menus ]
  ...
  function isEdiCentralVisible () {
    let visibleAppIdsArray = getUserData('apps') || []
    return visibleAppIdsArray.includes(52)
  }
  ...
  return (
    <SidebarTemplate
      ...
      searchBox={ searchBox }
      menus={ menus }
      ediCentral={ isEdiCentralVisible() }
    />
  )
}
