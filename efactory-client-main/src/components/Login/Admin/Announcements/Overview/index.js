...
const AnnouncementOverviewTab = props => {
	const [sortField, setSortField] = useState('published_from')
	const [sortDirection, setSortDirection] = useState('desc')

	useEffect(
		() => {
			props.loginActions.readOverviewArticles()
			return () => {
	...
	function onRowClicked (event) {
		...
		if (!received_id) {
			console.warn('No id found for onRowClicked method of Articles => ArticlesTable')
			return
		}
		received_id = +received_id
		let { announcements_overview_articles } = props.auth
		let index = announcements_overview_articles.findIndex(
			({ id }) => id === received_id
		)
		props.loginActions.setRootReduxStateProp_multiple({
			announcements_overview_articles_active_row_id: received_id,
			announcements_overview_articles_active_row: announcements_overview_articles[ index ]
		})
	}
  ...
  let {
	...
	announcements_overview_articles = sort(announcements_overview_articles)
    ...
    >
        ...
        <table className="table table-striped table-hover order-column row-sortable table-clickable table-accouncements">
          ...
          <tbody className="ui-sortable">
          	{
          		announcements_overview_articles.map( ( item, index ) => {
          			let {
          				id,
									title,
									author,
	          			...
	          			return (
	          				 <tr 
	          				 	className={ classNames({
            						...
            						'active' : +id === +announcements_overview_articles_active_row_id
            					}) } 
            					key={`article-${index}`}
            					data-item-id={ id }
