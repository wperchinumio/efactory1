...
const TransportationDeliveryTimes = ({
  reportData,
  analyticsState,
  analyticsActions,
}) => {
  ...
  function sortReportData () {  
    ...
    rows = rows.map( item => {
      ...
      return {
        ...item,
        total_packages : same_day + day1 + days2 + days3 + days4 + days5 +days6 +days7 + many_days
      }
    } )
              ...
              <thead>
                <tr className="uppercase analytics-tr">
                  ...
                  <th className="text-right" >
                    <span className="column-sort noprint"><i className={ classNames({
                      'fa fa-long-arrow-down' : true,
                      'active' : currentSortedField === 'days6' && sortType === "descending" ? true : false
                    }) } aria-hidden="true"></i></span>
                    <span className="column-sort column-sort-up noprint"><i className={ classNames({
                      'fa fa-long-arrow-up' : true,
                      'active' : currentSortedField === 'days6' && sortType === "ascending" ? true : false,
                    }) } aria-hidden="true"></i></span>
                    <a 
                      data-field="days6"
                      onClick={ sort }
                  ...
                  <th className="text-right" >
                    <span className="column-sort noprint"><i className={ classNames({
                      'fa fa-long-arrow-down' : true,
                      'active' : currentSortedField === 'days7' && sortType === "descending" ? true : false
                    }) } aria-hidden="true"></i></span>
                    <span className="column-sort column-sort-up noprint"><i className={ classNames({
                      'fa fa-long-arrow-up' : true,
                      'active' : currentSortedField === 'days7' && sortType === "ascending" ? true : false,
                    }) } aria-hidden="true"></i></span>
                    <a 
                      data-field="days7"
                      onClick={ sort }
                  ...
                  <th className="text-right" >
                    <span className="column-sort noprint"><i className={ classNames({
                      'fa fa-long-arrow-down' : true,
                      'active' : currentSortedField === 'total_packages' && sortType === "descending" ? true : false
                    }) } aria-hidden="true"></i></span>
                    <span className="column-sort column-sort-up noprint"><i className={ classNames({
                      'fa fa-long-arrow-up' : true,
                      'active' : currentSortedField === 'total_packages' && sortType === "ascending" ? true : false,
                    }) } aria-hidden="true"></i></span>
                    <a 
                      data-field="total_packages"
                      onClick={ sort }
                    > Packages With Delivery </a>
                  </th>

                  <th className="text-right" >
                    <span className="column-sort noprint"><i className={ classNames({
                      'fa fa-long-arrow-down' : true,
                      'active' : currentSortedField === 'not_delivered' && sortType === "descending" ? true : false
                    }) } aria-hidden="true"></i></span>
                    <span className="column-sort column-sort-up noprint"><i className={ classNames({
                      'fa fa-long-arrow-up' : true,
                      'active' : currentSortedField === 'not_delivered' && sortType === "ascending" ? true : false,
                    }) } aria-hidden="true"></i></span>
                    <a 
                      data-field="not_delivered"
                      onClick={ sort }
                    > Packages W/O Delivery </a>
                  </th>
              ...
              <tbody>
                  {
                    chartTable.map( ( item, index ) => {
                      ...
                      let total = same_day + day1 + days2 + days3 + days4 + days5 + days6 + days7 + many_days
                      ...
                      return (
                        <tr key={ `report-grid-${index}` } className="analytics-tr">
                          ...
                          <td className="text-right">
                            {
                              formatNumber( days6, 0 )
                            }
                          </td>
                          <td className="text-right">
                            {
                              formatNumber( days7, 0 )
                            }
                          </td>
                          <td className="text-right">
                            {
...
TransportationDeliveryTimes.propTypes = {
  reportData : PropTypes.object,
  analyticsState : PropTypes.object,
  analyticsActions : PropTypes.object,
}

export default TransportationDeliveryTimes