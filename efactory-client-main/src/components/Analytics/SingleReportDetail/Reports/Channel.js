...
const Report10 = ({
  ...
  function sort (event) {
  	...
  	analyticsActions.setRootReduxStateProp_multiple({
  		...
  		sortType: analyticsState.sortType === 'ascending' 
  							? 'descending'
  							: 'ascending'
		})
  }
  ...
  function changeCompareYears (event) {
	  analyticsActions.setRootReduxStateProp_multiple({
			...
			bars : event.target.checked
						 ? [...secondaryBars, ...mainBar]
						 : [...mainBar]
	  })
  }
  ...
  let total_1 = 0
  if (chart.length) {
  	total_1 = chart.map(r => r.year_1).reduce(
			( prev, next ) => {
				return {
					[ valueField ] : prev[ valueField ] + ( next[ valueField ] || 0  )
				} 
  ...
  let total_2 = 0
  if (chart.length) {
  	total_2 = chart.map(r => r.year_2).reduce( 
			( prev, next ) => {
				return {
					[ valueField ] : prev[ valueField ] + ( next[ valueField ] || 0  )
				} 
	      ...
	      <div className="row">
        	...
        	<div className="col-md-5">
              ...
              <span />
		        ...
		        <span>
						  <span className="total-edge channel-0">
						  	<b className="total">Total: &nbsp;</b> { formatNumber( total, 0 ) } 
						  </span>
						  <br/>
						  <span className="total-edge channel-1"> 
						  	<b className="total">Total (-1 y): &nbsp;</b> { formatNumber( total_1, 0 ) } 
						  </span>
						  <br/>
						  <span className="total-edge channel-2"> 
						  	<b className="total">Total (-2 y): &nbsp;</b> { formatNumber( total_2, 0 ) } 
						  </span>
						</span>
				  }
				</div>
						...
						<thead>
					        ...
					        <span className="column-sort column-sort-up noprint"><i className={ classNames({
					          ...
					          'active' : currentSortedField === 'name' && sortType === "ascending" ? true : false,
								...
								{ 
									compareYears && 
									<th className="text-right bl-light">
										Orders (-2)
									</th>
								}
								{ 
